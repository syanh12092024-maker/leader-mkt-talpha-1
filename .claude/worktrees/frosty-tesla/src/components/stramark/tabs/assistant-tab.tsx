...
export default function AssistantTab() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: "agent", content: "Xin ch├áo! T├┤i l├á Trß╗ú l├╜ AI cß╗ºa Stramark. T├┤i ─æ├ú ─æ╞░ß╗úc hß╗ìc to├án bß╗Ö quy tr├¼nh v├á h╞░ß╗¢ng dß║½n cß╗ºa bß║ín. Bß║ín cß║ºn t├¼m th├┤ng tin g├¼?" }
    ]);
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            if (data.error) {
                setMessages(prev => [...prev, { role: "agent", content: `Lß╗ùi: ${data.error}` }]);
            } else {
                // Formatting the raw output a bit if needed, or just displaying
                setMessages(prev => [...prev, { role: "agent", content: data.result || "Kh├┤ng t├¼m thß║Ñy th├┤ng tin ph├╣ hß╗úp." }]);
            }
        } catch (error) {
            setMessages(prev => [...prev, { role: "agent", content: "Lß╗ùi kß║┐t nß╗æi ─æß║┐n Server." }]);
        } finally {
            setLoading(false);
        }
    };
                ...
                {loading && (
                    <div className="flex w-full gap-3">
                        ...
                        <div className="flex items-center gap-2 rounded-lg bg-muted/50 px-4 py-2 text-sm text-foreground">
                            ...
                            <span>─Éang tra cß╗⌐u Knowledge Base...</span>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="border-t border-border p-4">
                <div className="flex gap-2">
                    <input
                        ...
                        placeholder="Hß╗Åi vß╗ü quy tr├¼nh, ch├¡nh s├ích, h╞░ß╗¢ng dß║½n UTM..."
