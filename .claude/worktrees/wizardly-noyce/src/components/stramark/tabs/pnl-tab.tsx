...
import { formatCurrency, formatMoney, COLORS, cn, formatNumber } from "@/lib/utils";
...
export default function PnLTab({ dateRange }: PnLTabProps) {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                ...
                // P&L from mart_performance_master — single source of truth
                const pnlQuery = `
                    SELECT 
                        report_date,
                        ...
                        ROUND(SUM(net_profit), 0) as profit
                ...
                let totalRev = 0, totalCogs = 0, totalAds = 0, totalShip = 0, totalProfit = 0;
                ...
                const dailyData = rows.map((r: any) => {
                    ...
                    const profit = r.profit || (rev - cogs - ads - ship);
                ...
                const waterfall = [
                    ...
                    { name: "Ship/FFM", value: -totalShip, fill: COLORS.indigo },
    ...
    const downloadCSV = () => {
        const headers = ["Date", "Orders", "Revenue", "COGS", "Ads Spend", "Ship/FFM", "Net Profit", "Margin %"];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <KPICard title="Ship/FFM (3PL)" value={formatCurrency(summary.shipping)} icon={Truck} />
                <KPICard
                    title="Net Profit"
                    ...
                    <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-card z-10">
                            <tr className="border-b border-border text-right text-muted-foreground">
                                ...
                                <th className="pb-3 text-indigo-400">Ship/FFM</th>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {daily.map((r, i) => (
                                <tr key={i} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="py-3 text-right text-indigo-300">-{formatMoney(r.shipping)}</td>
